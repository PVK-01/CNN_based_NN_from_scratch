...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    running_var = momentum * running_var + (1 - momentum) * sample_var
    a = x - sample_mean
    p = a / np.sqrt(sample_var + eps)
    out = gamma * p + beta
    cache = (out, p, a, momentum, sample_mean, sample_var, gamma, beta, eps)
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  out, p, a, momentum, sample_mean, sample_var, gamma, beta, eps = cache
  dgamma = np.sum(p * dout ,axis = 0)
  dbeta = np.sum(dout, axis = 0)
  dp = gamma * dout
  da_1 = (1/ np.sqrt(sample_var + eps)) * dp
  dmean_1 = np.sum(-da_1, axis = 0)
  dvar = np.sum((-1 / (2* (np.sqrt(sample_var + eps)** 3))) * dp * a , axis = 0)
  dx = (1 / (np.sqrt(sample_var + eps)) * dp) + ((1 / a.shape[0]) * dmean_1) + ((2 / a.shape[0]) * dvar * a)
